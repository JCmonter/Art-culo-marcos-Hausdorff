...
\section{El Q-cuadrado y cocientes compactos}
...
Además, $\mathbf{(H)}$ es conservativa, es decir, $S=\pt A$ es un espacio $T_2$. Al ser $S$ $T_2$, si $Q\in \mathcal{Q}S$, entonces $Q\in \mathcal{C}S$. Además,
\[
S \text{ es }T_2\Rightarrow S \text{ es fuertemente apilado}\Rightarrow v_\nabla=[Q'].
\]
...
\section{Marcos apilados y fuertemente apilados}
...
Trivialmente tenemos que fuertemente apilado implica apilado. Nuestro objetivo 
es demostrar que las propiedades anteriores son conservativas, es decir, que un espacio $S$ es 
apilado (fuertemente apilado) si y sólo si su marco de abiertos $\mathcal{O}S$ es apilado (fuertemente apilado). 
...
\section{Propiedades de separación}

Recordemos que si un espacio es $T_1$ y fuertemente apilado, entonces se satisface que $w_\nabla=v_\nabla$. Adicionalmente, en el caso espacial, también se cumple que
...
Antes de responder esto, necesitamos algunos resultados auxiliares. Si tomamos $p\in \pt A$ y el par HM $(F_p, \uparrow p)$, se cumple que para 
\[
...
Recordemos que por \eqref{Qlaxo}, para toda terna HM $(F, Q, \nabla)$ tenemos que 
\[
\mathcal{U}\circ v_F\leq v_\nabla \circ \mathcal{U}. 
\]

Además, si $A$ es fuertemente apilado, entonces $v_F=\mathcal{U}_*[Q']\mathcal{U}$ y por lo tanto
\[
\mathcal{U}_*\circ v_\nabla\circ \mathcal{U}\leq \mathcal{U}_*\circ [Q']\mathcal{U}
\]

Por lo tanto, $v_F=\mathcal{U}_*\circ v_\nabla\circ \mathcal{U}$ y, por propiedades del adjunto, $\mathcal{U}\circ v_F=v_\nabla\circ \mathcal{U}$.\\
...
Consideremos los siguientes hechos preliminares:

\begin{enumerate}
\item Consideremos a $A^\wedge$ como un espacio topológico donde los abiertos básicos de $\mathcal{O}(A^\wedge)$ están dados por 
\[
...
\section{Cocientes de marcos apilados}
