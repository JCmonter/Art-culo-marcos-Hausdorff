...
Conversely, let $A$ be efficient. Since $A$ is spatial, we may write
\[
A\cong\mathcal{O}(S)
\]
for some topological space $S$. By the spatial characterization of
efficiency, $S$ is packed and stacked. Moreover, efficiency implies
that $S$ is strongly stacked. Hence $S$ is packed and strongly
stacked, and therefore $\mathcal{O}(S)$ is KC. Thus $A$ is KC.
\end{proof}

\begin{lem}
For every topological space $S$,
\[
\mathcal{O}(S)\text{ is KC}
\quad\Longleftrightarrow\quad
S\text{ is packed and strongly stacked}.
\]
\end{lem}

\begin{proof}
Suppose first that $\mathcal{O}(S)$ is KC. Then $\mathcal{O}(S)$ is
efficient, and hence $S$ is packed and stacked. Since stacked and
strongly stacked are equivalent for KC-frames, $S$ is strongly
stacked.

Conversely, suppose that $S$ is packed and strongly stacked. Let
$(F,Q)$ be an HM-pair. Since $S$ is packed, $Q$ is closed and hence
$Q'\in\mathcal{O}(S)$. Therefore
\[
[Q']=u_{Q'}.
\]
Moreover, packed spaces are $T_1$, so that
\[
[Q']=w_F.
\]
Since $S$ is strongly stacked,
\[
v_F=[Q']=u_{Q'}=w_F.
\]
Thus
...
[v_F,w_F]=\{u_{Q'}\},
\]
and consequently every admissible nucleus is closed. Hence
$\mathcal{O}(S)$ is KC.
\end{proof}
