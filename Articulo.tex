...
\begin{enumerate}
    \item \emph{strongly stacked} if, for every HM pair $(F,Q)$,
    \[
    U_*\circ[Q']\circ U=v_F;
    \]
    
    \item \emph{stacked} if, for every HM pair $(F,Q)$,
    \[
...
Clearly, every strongly stacked frame is stacked. We now show that both
notions are conservative; namely, for every topological space $S$,
\[
S \text{ is stacked}
\Longleftrightarrow
...
Since $A$ is tidy, $\mathcal{O}S$ is tidy. Hence $S$ is stacked and packed.
...
v_\nabla(\varnothing)=Q'.
\]
Because $\mathcal{O}S$ is tidy, it follows that
\[
v_\nabla=u_{Q'}=[Q'].
\]
...
\begin{cor}
If $A$ is a tidy frame, then $\pt A$ is strongly stacked.
\end{cor}

\begin{proof}
Let $S=\pt A$. Since $A$ is tidy, $\mathcal{O}S$ is tidy, and therefore
...
p\leq a\leq q.
\]
By Proposition~6.1, every point of $A$ is maximal. Therefore, $p=q$, and
consequently
\[
p\leq a\leq p.
\]
...
v_F=\mathcal{U}_*\circ[Q']\circ\mathcal{U}.
\]
Moreover, since $A$ is $T_1$, we have $Q=M$. Hence, by Proposition~5.5,
\[
v_F
...
Notice that the additional hypothesis in Proposition \ref{IsoVA} (3) is automatically
...
spatiality. We now consider a stronger separation property that provides
another source of stacked frames.

Recall that a frame $A$ satisfies property $\mathbf{(H)}$ if
\[
1\neq a\nleq b
\]
...
\neg c\nleq b.
\]
As shown in \cite{PasekaSmarda1992}, property $\mathbf{(H)}$ implies $T_1$.

\begin{prop}
Every frame satisfying property $(H)$ is stacked.
\end{prop}
...
Consequently, every frame satisfying $\mathbf{(H)}$ is both $T_1$ and stacked.
In particular, the non-spatial frames satisfying $\mathbf{(H)}$ constructed by
Paseka and Šmarda provide another family of non-spatial $T_1$ stacked
frames.

Thus, the point-free setting exhibits a behavior that has no direct spatial
counterpart: the $T_1$ and stacked conditions do not force spatiality. This
occurs both in the strongly stacked example above and, more generally, among
non-spatial frames satisfying property $\mathbf{(H)}$.
