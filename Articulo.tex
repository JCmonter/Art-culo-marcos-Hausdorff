...
\begin{tikzcd}
    & {\mathbf{(H)+(sfit)}} & \\
    {\mathbf{(reg)}} & {\mathbf{(sH)}} & {T_1} \\
    & {\mathbf{(fit)}} & {\mathbf{(sfit)}}
...
\section{Compact Quotients and Admissibility Intervals}
\begin{itemize}
...
Every tidy strongly stacked frame is KC.
\end{prop}
...
$(H)$. Recall that a frame $A$ satisfies property $(H)$ if
\[
    1\neq a\nleq b
\]
...
\[
    ...
    \neg c\nleq b.
\]
As shown in \cite{PasekaSmarda1992}, property $(H)$ implies $T_1$. Nevertheless, property
$(H)$ does not imply stackedness, as the following example shows.

\begin{ej}
There exists a frame satisfying property $(H)$ which is not stacked.
...
\[
    \widetilde{A}
    =
    \{(u,v)\in A\times \mathfrak{B}(A):u\leq v\},
\]
where
\[
    \mathfrak{B}(A)=\{a^{**}:a\in A\}
\]
...
In particular, $\widetilde{A}$ satisfies property $(H)$.
...
Since $\widetilde{A}$ satisfies $(H)$, it is $T_1$, and hence its points
...
Consequently, although property (H) implies $T_1$, it does not imply
...
stronger Hausdorff condition (H) does not guarantee stackedness.
